...
const BulkStudentImport = ({
  ...
  const downloadTemplate = () => {
    ...
    const instructions = [
      ...
      { Column: "Phone", Required: "Yes", Description: "Phone number" },
      {
        Column: "Course ID",
        Required: "Yes",
        Description: "Select from available list",
      },
  ...
  const generateUniqueRollNumber = (courseName, admissionDate, attempt = 0) => {
    ...
    if (existingRollNumbers.has(rollNumber)) {
      rollNumber = `${generateRollNumber(courseName, admissionDate)}+${
        attempt + 1
      }`;
    }
  ...
  const validateAndPreview = (data) => {
    ...
    data.forEach((row, index) => {
      ...
      while (
        ...
        usedRollNumbers.has(rollNumber)
      ) {
        attempt++;
      ...
      const duplicateEmail = validatedData.find(
        (s) => s.email === row["Email"].trim()
      );
      if (duplicateEmail) {
        errorList.push(
          `Row ${rowNum}: Duplicate email "${row["Email"]}" found in batch`
        );
        return;
      }
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        <div className="bg-gray-600 dark:bg-gray-700 text-white p-6 flex items-center justify-between">
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)] space-y-6">
          {/* Course Information */}
          <div className="bg-gray-50 dark:bg-gray-900/30 p-4 border-l-4 border-gray-500 rounded-r-lg">
            <h3 className="font-semibold text-gray-900 dark:text-gray-300 mb-3">
              Available Courses
            </h3>
            ...
            <p className="text-sm text-gray-700 dark:text-gray-300 mt-3">
              Use these course IDs in the Excel file.
            </p>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 p-8 text-center rounded-lg hover:border-gray-500 transition-colors">
              <input
                type="file"
                accept=".xlsx,.xls"
          ...
          {success && (
            <div className="bg-green-50 dark:bg-green-900/30 p-4 border-l-4 border-green-500 rounded-r-lg flex items-center gap-3">
              ...
              <p className="text-green-700 dark:text-green-300 font-semibold">
                ✓ Import successful! {preview.length} students added.
              </p>
            </div>
          )}
          ...
          {preview.length > 0 && errors.length === 0 && (
            <div>
              <h3 className="font-semibold text-gray-900 dark:text-gray-100 mb-3">
                Step 3: Review Data ({preview.length} students)
              </h3>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-3 bg-gray-50 dark:bg-gray-900">
          ...
          <button
            ...
            className="px-6 py-2 bg-gray-600 hover:bg-gray-700 dark:bg-gray-700 dark:hover:bg-gray-600 text-white rounded-lg flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
            {importing ? (
